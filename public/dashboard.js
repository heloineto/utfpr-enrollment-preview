...
const subjectsHtml = Object.entries(subjects).map((subject) => {
  const subjectName = subject[0];
  const utfClasses = subject[1];

  const utfClassesHtml = Object.entries(utfClasses).map((utfClasse) => {
    const classCode = utfClasse[0];
    const classData = utfClasse[1];

    return `
          <tr id="${subjectName};;${classCode}" class="utfClass hover:bg-yellow-100 cursor-pointer hover:shadow-lg hover-dark-shadow transition-all	duration-300">
            <td class="px-6 py-4 text-sm font-medium text-gray-900">
              ${classCode}
            </td>
            <td class="px-6 py-4 text-sm text-gray-900">
              ${classData.schedule.map((each) => each.timeCode).join(', ')}
            </td>
            <td class="px-6 py-4 text-sm text-gray-900">
              ${classData.professor}
            </td>
            <td class="px-6 py-4 text-sm text-gray-500">
              ${classData.vacanciesTotal}
            </td>
            <td class="px-6 py-4 text-gray-500 text-sm text-right break-all w-40"
            >
              ${classData.optional}
            </td>
          </tr>
  ...
  return `
      <div id="${subjectName}" class="discipline"> 
        <tr id="${subjectName}-header" class="bg-gray-50 w-full relative subject-header">
          <td class="h-12">
...
subjectsTableBody.innerHTML = subjectsHtml.join('');

const selectedIds = {};

const utfClasses = document.querySelectorAll('.utfClass');
const classesList = document.getElementById('classesList');
const classesJsonInput = document.getElementById('classesJsonInput');

const handleclick = (e) => {
  const classId = e.currentTarget.id;
  const subjectName = classId.split(';;')[0];
  const className = classId.split(';;')[1];

  if (selectedIds[classId]) {
    e.currentTarget.classList.remove('selected');
    delete selectedIds[classId];

    document.getElementById(`listEntry;;${subjectName};;${className}`).remove();
    classesJsonInput.setAttribute(
      'value',
      JSON.stringify(
        Object.keys(selectedIds).map((key) => ({
          subject: key.split(';;')[0],
          classCode: key.split(';;')[1],
        }))
      )
    );
  ...
  selectedIds[classId] = subjects[subjectName][className];

  const listEntry = document.createElement('li');

  listEntry.setAttribute('id', `listEntry;;${subjectName};;${className}`);
  ...
  listEntry.innerHTML = `
    <div class="py-4 flex items-center space-x-4">
      ...
      <div>
        <a href="#${classId}" class="inline-flex items-center shadow-sm bg-gray-900 px-2.5 py-0.5 border border-gray-300 text-sm leading-5 font-medium rounded-full text-gray-200 bg-white hover:bg-black">
          Navegar Até
        </a>
      </div>
    </div>
  ...
  classesList.appendChild(listEntry);
};
...
//   const getColor = () => {
//     const subjectCode = subjectHeader.id.split(' - ')[0];

//     if(!subjectCode.startsWith('CC'))
//       return '#FFE4E6';

//     switch(subjectCode[3]) {
//       case '1': return '#FCE7F3'
//       case '2': return '#EDE9FE'
//       case '3': return '#E0E7FF'
//       case '4': return '#DBEAFE'
//       case '5': return '#CFFAFE'
//       case '6': return '#D1FAE5'
//       case '7': return '#FFEDD5'
//       default: return '#FEE2E2'
//     }
//   }

//   subjectHeader.style.backgroundColor = getColor()
// })
